...
- Google Trends 資料已內嵌在 `app.py`，不自動抓取 Google Trends，也不需要使用者上傳或建立 `data/` 資料夾
...
三份 Google Trends CSV 已用 base64 內嵌在 `app.py`。部署到 Streamlit Cloud 時，只需要確保 GitHub repo 有 `app.py` 與 `requirements.txt`。
